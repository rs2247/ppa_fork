<template>
  <div class="container-fluid">
    ...
    <modal-dialog ref="boModal" :identification="stock-modal">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h2 class="modal-title">DETALHES DO BO</h2>
      </div>
      <div class="modal-body" v-if="currentBoData">
        <div class="transparent-loader" v-if="boDataLoading">
          <div class="loader"></div>
        </div>
        <label>CPF:</label>
        <p>{{ currentBoData.cpf }}</p>
        <label>OBSERVAÇÔES</label>
        <p>{{ currentBoData.observations }}</p>
        <label>COMENTÁRIOS</label>
        <template v-if="boComments">
          <template v-if="boComments.length > 0">
            <p v-for="comment in boComments">
              ...
              <table id="bo-table" class="data-table data-table-tiny data-table-no-wrap">
                <thead>
                  <tr>
                    ...
                    <th>Campus</th>
                    <th>Motivo</th>
                    <th><span class="small-padding-right">Motivo Final</span></th>
                ...
                <tbody>
                  <tr v-for="entry in tableData" class="clickable" @click="showBoModal(entry)">
...
export default {
  ...
  mounted() {
    BoChannel.on('filters', (payload) => this.receiveFilters(payload));
    BoChannel.on('tableData', (payload) => this.receiveTableData(payload));
    BoChannel.on('boData', (payload) => this.receiveBoData(payload));
  ...
  methods: {
    ...
    showBoModal(boData) {
      ...
      BoChannel.push('loadBoData', { operation_id: boData.id }).receive('timeout', (data) => {
    ...
    loadFilters() {
      BoChannel.push('loadFilters').receive('timeout', (data) => {
    ...
    loadData() {
      ...
      BoChannel.push('loadData', filter).receive('timeout', (data) => {
